...
struct process {
  ...
  int priority; //for priority sechduling
};

void completion_of_process(struct process* pro, int curr_time, int type) {
  if(type==1)
  {
    pro->completion_time=curr_time;
  }
  else
...
void fcfs(struct process* processes, int n)
{
  ...
  while(front<= rear)
  {
    ...
    completion_of_process(&p, current_time, 0);
...
void PriorityScheduling(struct process* processes, int n) {
  ...
  while (completed < n) {
    ...
    for (int i = 0; i < n; i++) {
      if (processes[i].arrival_time <= current_time && processes[i].completion_time == 0) { //Priority =5 high ; priority=1 low
  ...
  while (completed < n) {
    for (int i = 0; i < n; i++) {
      if (processes[i].remaining_time > 0) {
        ...
        if (processes[i].remaining_time == 0) {
          completion_of_process(&processes[i], current_time, 1);
...
int main() {
  ...
  struct process processes1[] = {
    {1,0,3,0,0,0,3,3}, // Priority Scheduling: priority = 3
    {2,1,4,0,0,0,4,2}, // Priority Scheduling: priority = 2
    {3,2,6,0,0,0,6,5}, // Priority Scheduling: priority = 5
    {4,3,2,0,0,0,2,1}, // Priority Scheduling: priority = 1
    {5,4,7,0,0,0,7,4}  // Priority Scheduling: priority = 4
  };
  printf("\nPriority Scheduling:\n");
  PriorityScheduling(processes1, n);

  struct process processes2[] = {
    {1,0,3,0,0,0,3}, // Priority Scheduling: priority = 2
    {2,1,4,0,0,0,4}, // Priority Scheduling: priority = 1
    {3,2,6,0,0,0,6}, // Priority Scheduling: priority = 3
    {4,3,2,0,0,0,2}, // Priority Scheduling: priority = 4
    {5,4,7,0,0,0,7}  // Priority Scheduling: priority = 5
  };

  printf("\nRound Robin:\n");
  RoundRobin(processes2, n, time_quantum);
